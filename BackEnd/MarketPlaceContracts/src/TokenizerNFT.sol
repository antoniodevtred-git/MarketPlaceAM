...
contract TokenizerNFT is ERC721URIStorage, Ownable {
    ...
    event NFTTokenized(uint256 indexed tokenId_, uint256 indexed itemId_, address indexed owner_, string uri_);

    constructor(string memory name_, string memory symbol_, address owner_) ERC721(name_, symbol_) Ownable(owner_) {}

    function tokenize(address to_, uint256 itemId_, string calldata uri_) external onlyOwner returns (uint256 tokenId_) {

        require(to_ != address(0), "12");
        require(itemToToken[itemId_] == 0, "02"); // ITEM_EXISTS
